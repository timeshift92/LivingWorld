...
| P1-C-followup | **Codex (Core) follow-up surfaced by P1-R1:** add `WorldState.LaunchRaidPreparation` (mark Launched without releasing the deployed army — the enum has `Launched` but no transition), and wire `RaidPreparationService.ReleaseExpiredPreparations` into the daily tick so stale Ready preparations are cleaned. Not blocking (P1-R1 uses `Release` safely), but needed for a correct lifecycle. | Core/Codex | — | ▢ |
