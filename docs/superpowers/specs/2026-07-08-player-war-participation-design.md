...
- **Slice 1 — DONE** (Core model reworked per Codex's design): `PlayerConflictInterventionService.RecordSettlementAttack` pressures the **victim** faction in every war it is fighting (raising its exhaustion, claiming a captured settlement once) so the player's blows actually help whoever the victim fights — including an ally — **without** making the player a belligerent or an ally; a peacetime attack records a standalone aggression event for later diplomacy/revenge. RW `LivingWorldSettlementDefeatPatch` (reflection-verified hook, dedups by world-object ID) calls it. This closes the Slice 4 loop (attacks now move the ally-vs-enemy war's exhaustion). Replaced the earlier simpler `PlayerBelligerenceService`. Unit + structural tests.
