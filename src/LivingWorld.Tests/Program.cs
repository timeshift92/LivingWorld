...
var tests = new List<(string Name, Action Test)>
{
    ...
    ("destroyed settlements stop animal production and breeding", TestDestroyedSettlementStopsAnimalSimulation),
    ...
    ("simulates only active settlements and desynchronizes birth cadence", TestSettlementDailySimulationSkipsInactiveAndDesynchronizesBirths),
    ...
    ("reroutes migration when its destination becomes invalid", TestMigrationReroutesInvalidTarget),
    ("returns migration people and cargo when no reroute exists", TestMigrationReturnsPeopleAndCargo),
    ("records explicit lost people and cargo when migration has no refuge", TestMigrationLosesPeopleAndCargoExplicitly),
    ("moves refugees stranded by settlement destruction", TestDestroyedSettlementRefugeesResumeMigration),
    ...
    ("raid allocation reserves real equipment materials", TestRaidPopulationAllocatorReservesEquipment),
    ...
    ("settlement materialization stays with a captured settlement", TestMaterializationLeaseStaysWithCapturedSettlement),
    ...
    ("materialization lease cannot be rebound to another pawn", TestMaterializationLeaseRejectsPawnRebind),
    ...
    ("collapses a faction after its last settlement is captured", TestFactionLifecycleCollapsesAfterLastCapture),
    ...
    ("terminal armies release dead citizen ownership", TestTerminalArmyReleasesDeadOwnership),
    ...
    ("scouts recall when target ownership or alliance changes", TestWorldMissionRecallsInvalidScoutTargets),
    ...
    ("aborted raids return people and cargo to a surviving faction settlement", TestRaidRecallAvoidsCapturedSource),
    ...
    ("persistent caravan preserves crew through save load", TestPersistentCaravanCrewSerialization),
    ("caravan recalls crew and cargo when target is inactive", TestCaravanRecallsWhenTargetInactive),
    ...
    ("caravans recall instead of trading with a hostile captor", TestCaravanRecallsFromHostileCaptor),
    ...
    ("change settlement faction no-op when not active", TestChangeSettlementFactionIsNoOpWhenNotActive),
    ...
    ("applies knowledge fog and physical discovery to world traffic", TestRimWorldWorldTrafficFogOfWar),
    ...
    ("keeps loaded NPC settlement maps live and save-stable", TestRimWorldSettlementVisitLiveLifecycle),
...
static void TestAnimalEcologyDriverSeedsFromSettlementEnvironment()
{
    ...
    AssertEqual(2, result.CohortsSeeded);
    AssertEqual(false, desertCohorts.Any(cohort => cohort.Type == AnimalCohortType.Domesticated));
    AssertEqual(true, desertCohorts.Any(cohort => cohort.Type == AnimalCohortType.Wild && cohort.AnimalKind == "Ibex"));
    AssertEqual(false, borealCohorts.Any(cohort => cohort.Type == AnimalCohortType.Domesticated));
...
static void TestAnimalEcologyDriverIsIdempotentAndFeedsDomesticatedCohorts()
{
    ...
    AssertEqual(1, dayOne.CohortsSeeded);
...
static void TestSettlementDailySimulationConsumesFoodAndBirths()
{
    ...
    var dayOne = SettlementDailySimulationService.SimulateDay(
        state,
        new SettlementDailySimulationRequest(60_000, "PackagedSurvivalMeal", 1, 1));

    AssertEqual(2, dayOne.FoodConsumed);
    AssertEqual(1, dayOne.Births);
...
static void TestSettlementDailySimulationSkipsInactiveAndDesynchronizesBirths()
{
    var state = new WorldState(24680);
    var active = new List<WorldSettlement>();
    for (var index = 0; index < 6; index++)
    {
        var settlement = state.CreateSettlement($"active-{index}", $"Active {index}", "Settlers");
        state.CreateCitizen($"Parent A {index}", 28, Sex.Female, "settler", settlement.Id);
        state.CreateCitizen($"Parent B {index}", 30, Sex.Male, "settler", settlement.Id);
        state.AddResource(settlement.Id, "Food", 100);
        active.Add(settlement);
    }

    var inactive = state.CreateSettlement("inactive", "Inactive", "Settlers");
    state.CreateCitizen("Inactive parent A", 28, Sex.Female, "settler", inactive.Id);
    state.CreateCitizen("Inactive parent B", 30, Sex.Male, "settler", inactive.Id);
    state.AddResource(inactive.Id, "Food", 100);
    SettlementLifecycleService.DestroySettlement(state, inactive.Id, 1, "test inactive settlement");
    var inactiveFood = state.GetOwnedResourceQuantity(inactive.Id, "Food");

    var birthDays = new Dictionary<EntityId, int>();
    var previousPopulation = active.ToDictionary(
    ...
    for (var day = 1; day <= 7; day++)
    {
        SettlementDailySimulationService.SimulateDay(
            state,
            new SettlementDailySimulationRequest(day * 60_000, "Food", 0, 7));
        foreach (var settlement in active)
        {
            var population = state.GetSettlementPopulation(settlement.Id).Total;
            if (population > previousPopulation[settlement.Id])
            {
                birthDays[settlement.Id] = day;
            }

            previousPopulation[settlement.Id] = population;
        }
    }

    AssertEqual(active.Count, birthDays.Count);
    AssertEqual(true, birthDays.Values.Distinct().Count() > 1);
    AssertEqual(inactiveFood, state.GetOwnedResourceQuantity(inactive.Id, "Food"));
    AssertEqual(0, state.Events.Count(worldEvent =>
        worldEvent.Kind == WorldEventKind.CitizenBorn
        && worldEvent.SubjectId.HasValue
        && state.GetCitizen(worldEvent.SubjectId.Value)?.SettlementId == inactive.Id));
}
...
static void TestSettlementBootstrapPrimerCreatesVisibleDynamics()
{
    ...
    AssertEqual(0, result.FacilitiesSeeded);
    ...
    AssertEqual(true, state.GetAnimalCohorts(settlement.Id).All(cohort => cohort.Type == AnimalCohortType.Wild));
...
static void TestMigrationPressureCreatesRefugee()
{
    ...
    AssertEqual(4, state.GetSettlementPopulation(source.Id).Total);
    AssertEqual(0, statusAfter.Refugees);
    AssertEqual(true, state.Citizens.All(citizen => citizen.Status == CitizenStatus.Alive));
    AssertEqual(true, state.Citizens.All(citizen => state.GetOwner(citizen.Id) == source.Id));
    AssertEqual(0, state.Events.Count(worldEvent => worldEvent.Kind == WorldEventKind.RefugeeCreated));
...
static void TestMigrationReroutesInvalidTarget()
{
    var state = new WorldState(12345);
    var source = state.CreateSettlement("source", "Source", "Settlers");
    var firstTarget = state.CreateSettlement("first", "First", "Settlers");
    var secondTarget = state.CreateSettlement("second", "Second", "Settlers");
    var migrant = state.CreateCitizen("Migrant", 24, Sex.Female, "settler", source.Id);
    state.CreateCitizen("First resident", 30, Sex.Male, "settler", firstTarget.Id);
    state.CreateCitizen("Second resident", 30, Sex.Male, "settler", secondTarget.Id);
    state.AddResource(firstTarget.Id, "Food", 40);
    state.AddResource(secondTarget.Id, "Food", 20);

    MigrationService.SimulateDay(state, new MigrationSimulationRequest(60_000, "Food", 1, 50, 1, 60_000));
    var group = state.MigrationGroups.Single();
    AssertEqual(firstTarget.Id, group.TargetSettlementId);
    SettlementLifecycleService.DestroySettlement(state, firstTarget.Id, 90_000, "target destroyed");

    MigrationService.SimulateDay(state, new MigrationSimulationRequest(120_000, "Food", 1, 50, 0, 60_000));
    var rerouted = state.GetMigrationGroup(group.Id)!;
    AssertEqual(MigrationGroupStatus.Traveling, rerouted.Status);
    AssertEqual(secondTarget.Id, rerouted.TargetSettlementId);
    AssertEqual(180_000, rerouted.ArrivalTick);
    AssertEqual(group.Id, state.GetOwner(migrant.Id));

    var completed = MigrationService.SimulateDay(state, new MigrationSimulationRequest(180_000, "Food", 1, 50, 0, 60_000));
    AssertEqual(1, completed.MigrationsCompleted);
    AssertEqual(secondTarget.Id, state.GetOwner(migrant.Id));
    AssertEqual(0, state.Validate().Count());
}

static void TestMigrationReturnsPeopleAndCargo()
{
    var state = new WorldState(12345);
    var source = state.CreateSettlement("source", "Source", "Settlers");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    var migrant = state.CreateCitizen("Migrant", 24, Sex.Female, "settler", source.Id);
    state.CreateCitizen("Target resident", 30, Sex.Male, "settler", target.Id);
    state.AddResource(target.Id, "Food", 20);

    MigrationService.SimulateDay(state, new MigrationSimulationRequest(60_000, "Food", 1, 50, 1, 60_000));
    var group = state.MigrationGroups.Single();
    state.AddResource(group.Id, "Steel", 12);
    SettlementLifecycleService.DestroySettlement(state, target.Id, 90_000, "target destroyed");

    MigrationService.SimulateDay(state, new MigrationSimulationRequest(120_000, "Food", 1, 50, 0, 60_000));

    AssertEqual(MigrationGroupStatus.Arrived, state.GetMigrationGroup(group.Id)!.Status);
    AssertEqual(source.Id, state.GetMigrationGroup(group.Id)!.TargetSettlementId);
    AssertEqual(CitizenStatus.Alive, state.GetCitizen(migrant.Id)!.Status);
    AssertEqual(source.Id, state.GetOwner(migrant.Id));
    AssertEqual(12, state.GetOwnedResourceQuantity(source.Id, "Steel"));
    AssertEqual(0, state.GetOwnedResourceQuantity(group.Id, "Steel"));
    AssertEqual(0, state.Validate().Count());
}

static void TestMigrationLosesPeopleAndCargoExplicitly()
{
    var state = new WorldState(12345);
    var source = state.CreateSettlement("source", "Source", "Settlers");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    ...
    var group = state.MigrationGroups.Single();
    state.AddResource(group.Id, "Steel", 12);
    SettlementLifecycleService.DestroySettlement(state, source.Id, 90_000, "source destroyed");
    SettlementLifecycleService.DestroySettlement(state, target.Id, 90_000, "target destroyed");

    MigrationService.SimulateDay(state, new MigrationSimulationRequest(120_000, "Food", 1, 50, 0, 60_000));

    AssertEqual(MigrationGroupStatus.Lost, state.GetMigrationGroup(group.Id)!.Status);
    AssertEqual(CitizenStatus.Missing, state.GetCitizen(migrant.Id)!.Status);
    AssertEqual(migrant.Id, state.GetOwner(migrant.Id));
    AssertEqual(0, state.GetOwnedResourceQuantity(group.Id, "Steel"));
    AssertEqual(0, state.Validate().Count());
}
...
static void TestMaterializationLeaseStaysWithCapturedSettlement()
{
    var state = new WorldState(12345);
    ...
    AssertEqual(0, state.GetOwnedResourceQuantity(fallback.Id, "Steel"));
    AssertEqual(10, state.GetOwnedResourceQuantity(source.Id, "Steel"));
}
...
static void TestOpposingArmiesInterceptInTransit()
{
    ...
    FactionConflictPolicy.DeclareWar(state, "Red", "Blue", 0);
...
static void TestHostileArmiesInterceptCaravansInTransit()
{
    ...
    FactionConflictPolicy.DeclareWar(state, "Raiders", "Traders", 0);
...
static void TestHostileArmiesContestSameTargetInTransit()
{
    var state = new WorldState(4242);
    var redHome = state.CreateSettlement("red-home", "Red Home", "Red");
    var blueHome = state.CreateSettlement("blue-home", "Blue Home", "Blue");
    var prize = state.CreateSettlement("prize", "Prize", "Prize");
    for (var i = 0; i < 6; i++)
    {
        state.CreateCitizen("R" + i, 30, Sex.Male, "fighter", redHome.Id);
        state.CreateCitizen("B" + i, 30, Sex.Male, "fighter", blueHome.Id);
    ...
    FactionConflictPolicy.DeclareWar(state, "Red", "Blue", 0);
    state.DispatchArmy(redReserve.Army!.Id, prize.Id, arrivalTick: 5 * 60_000);
    state.DispatchArmy(blueReserve.Army!.Id, prize.Id, arrivalTick: 5 * 60_000);
...
static void TestHostileArmiesInterceptSameTargetCaravansInTransit()
{
    var state = new WorldState(4242);
    var traderHome = state.CreateSettlement("trader-home", "Trader Home", "Traders");
    var raiderHome = state.CreateSettlement("raider-home", "Raider Home", "Raiders");
    var market = state.CreateSettlement("market", "Market", "Market");
    state.AddResource(traderHome.Id, "Steel", 50);
    var crew = state.CreateCitizen("Crew", 30, Sex.Female, "merchant", traderHome.Id);
    for (var i = 0; i < 6; i++)
    {
        state.CreateCitizen("R" + i, 30, Sex.Male, "raider", raiderHome.Id);
    }

    var caravan = state.CreateCaravan("Steel caravan", "Traders", traderHome.Id, market.Id, 0, 5 * 60_000, crew.Id);
    state.TransferAsset(crew.Id, traderHome.Id, caravan.Id, "caravan crew");
    state.TransferResource(traderHome.Id, caravan.Id, "Steel", 20, "cargo");
    var reservation = RaidPopulationAllocator.ReserveForRaid(
        state,
        new RaidPopulationAllocationRequest("Raiders", "Road patrol", 4, FoodPerCitizen: 0));
    FactionConflictPolicy.DeclareWar(state, "Raiders", "Traders", 0);
    state.DispatchArmy(reservation.Army!.Id, market.Id, 5 * 60_000);
    ...
    AssertEqual(traderHome.Id, state.GetOwner(crew.Id));
    AssertEqual(0, state.Validate().Count());
}
...
static void TestHostileArmiesDisruptMissionsInTransit()
{
    ...
    var reservation = RaidPopulationAllocator.ReserveForRaid(
        state,
        new RaidPopulationAllocationRequest("Raiders", "Road patrol", 4, FoodPerCitizen: 0));
    FactionConflictPolicy.DeclareWar(state, "Raiders", "Scouts", 0);
...
static void TestWorldMissionReservesAndReturnsRealCitizen()
{
    var state = new WorldState(4242);
    var scouts = state.CreateSettlement("scouts", "Scouts", "Scouts");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    var citizen = state.CreateCitizen("Scout", 30, Sex.Female, "scout", scouts.Id);

    var mission = state.DispatchMission(WorldMissionKind.Scout, "Scouts", scouts.Id, target.Id, 0, 60_000, amount: 100, crewCitizenId: citizen.Id);
    state.TransferAsset(citizen.Id, scouts.Id, mission.Id, "scouting mission launched");

    AssertEqual(mission.Id, state.GetOwner(citizen.Id));
    AssertEqual(0, state.GetSettlementPopulation(scouts.Id).Total);

    WorldMissionService.SimulateDay(state, new WorldMissionRequest(60_000));
    AssertEqual(WorldTransitPhase.AtTarget, state.GetMission(mission.Id)!.Phase);
    AssertEqual(mission.Id, state.GetOwner(citizen.Id));
...
static void TestWorldMissionFailsWhenTargetInactive()
{
    var state = new WorldState(4242);
    var scouts = state.CreateSettlement("scouts", "Scouts", "Scouts");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    ...
    var mission = state.DispatchMission(WorldMissionKind.Scout, "Scouts", scouts.Id, target.Id, 0, 60_000, amount: 100, crewCitizenId: citizen.Id);
    state.TransferAsset(citizen.Id, scouts.Id, mission.Id, "scouting mission launched");
    SettlementLifecycleService.DestroySettlement(state, target.Id, 30_000, "destroyed before arrival");

    WorldMissionService.SimulateDay(state, new WorldMissionRequest(60_000));

    AssertEqual(WorldMissionStatus.Traveling, state.GetMission(mission.Id)!.Status);
    AssertEqual(WorldTransitPhase.Returning, state.GetMission(mission.Id)!.Phase);
    AssertEqual(mission.Id, state.GetOwner(citizen.Id));
    AssertEqual(null, state.GetKnownSettlementInfo(target.Id));
...
static void TestWorldMissionRecallsInvalidScoutTargets()
{
    var state = new WorldState(4242);
    var scouts = state.CreateSettlement("scouts", "Scouts", "Scouts");
    var capturedTarget = state.CreateSettlement("captured", "Captured Target", "Settlers");
    var alliedTarget = state.CreateSettlement("allied", "Allied Target", "Neighbors");
    var captureScout = state.CreateCitizen("Capture Scout", 30, Sex.Female, "scout", scouts.Id);
    var allianceScout = state.CreateCitizen("Alliance Scout", 31, Sex.Male, "scout", scouts.Id);

    var captureMission = state.DispatchMission(
        ...
        60_000,
        amount: 100,
        crewCitizenId: captureScout.Id);
    var allianceMission = state.DispatchMission(
        WorldMissionKind.Scout,
        "Scouts",
        scouts.Id,
        alliedTarget.Id,
        ...
        60_000,
        amount: 100,
        crewCitizenId: allianceScout.Id);
    state.TransferAsset(captureScout.Id, scouts.Id, captureMission.Id, "scouting mission launched");
    state.TransferAsset(allianceScout.Id, scouts.Id, allianceMission.Id, "scouting mission launched");

    state.CaptureSettlement(capturedTarget.Id, "Scouts");
    DiplomacyService.AdjustGoodwill(state, "Scouts", "Neighbors", 80);

    WorldMissionService.SimulateDay(state, new WorldMissionRequest(60_000));

    AssertEqual(WorldTransitPhase.Returning, state.GetMission(captureMission.Id)!.Phase);
    AssertEqual(WorldTransitPhase.Returning, state.GetMission(allianceMission.Id)!.Phase);
    AssertEqual(false, state.GetMission(captureMission.Id)!.ReportCollected);
    AssertEqual(false, state.GetMission(allianceMission.Id)!.ReportCollected);

    WorldMissionService.SimulateDay(state, new WorldMissionRequest(120_000));

    AssertEqual(WorldMissionStatus.Failed, state.GetMission(captureMission.Id)!.Status);
    AssertEqual(WorldMissionStatus.Failed, state.GetMission(allianceMission.Id)!.Status);
    AssertEqual(scouts.Id, state.GetOwner(captureScout.Id));
    AssertEqual(scouts.Id, state.GetOwner(allianceScout.Id));
    AssertEqual(false, state.HasFactionSettlementIntel("Scouts", capturedTarget.Id));
    AssertEqual(false, state.HasFactionSettlementIntel("Scouts", alliedTarget.Id));
    AssertEqual(0, state.Validate().Count());
}

static void TestNpcScoutPlayerContactRoundTrip()
{
    var state = new WorldState(4242);
    var source = state.CreateSettlement("scouts", "Scout Post", "Raiders");
    for (var index = 0; index < 6; index++)
    {
        state.CreateCitizen("Scout " + index, 30, Sex.Male, "scout", source.Id);
    }
    ...
    var mission = state.Missions.Single(candidate => candidate.Kind == WorldMissionKind.Scout);
    var crewId = mission.CrewCitizenId!.Value;
    AssertEqual(true, mission.TargetsPlayerContact);
    AssertEqual("worldtile:77", mission.TargetContactKey);
    AssertEqual(mission.Id, state.GetOwner(crewId));
    AssertEqual(0, FactionKnowledgeService.GetActiveRaidIntelFacts(state, "Raiders").Count);

    var restored = WorldStateCodec.Deserialize(WorldStateCodec.Serialize(state));
    WorldWarService.SimulateDay(restored, new WorldWarRequest(120_000, TravelDays: 1, RaidCombatants: 3));
...
static void TestWorldWarNonCombatActionsSkipPlayerFaction()
{
    var state = new WorldState(4242);
    state.SetPlayerFactionId("PlayerFaction");
    state.SetPlayerContactEndpoint("PlayerFaction", "player-contact:home", true, 0);
    ...
    AssertEqual(village.Id, WorldWarTargetSelector.FindTradeTarget(state, "Traders")!.Id);
    ...
    var scoutTarget = WorldWarTargetSelector.FindScoutingTarget(state, "Scouts");
    AssertEqual(true, scoutTarget != null);
    AssertEqual(false, state.IsPlayerFaction(scoutTarget!.FactionId));
    ...
    state.AssignFactionBehavior("Envoys", FactionBehavior.Random);
    var diplomacy = FactionActionPlanner.Plan(state, "Envoys", 4 * 60_000);
    AssertEqual(WarAction.Diplomat, diplomacy.Action);
    AssertEqual(null, diplomacy.TargetSettlementId);
    AssertEqual("PlayerFaction", diplomacy.TargetFactionId);
    AssertEqual(0, state.Settlements.Count(settlement => state.IsPlayerFaction(settlement.FactionId)));
}
...
static void TestWorldWarLaunchesAndResolvesWarband()
{
    ...
    state.AddResource(horde.Id, "PackagedSurvivalMeal", 100);
    ...
    AssertEqual(
        DiplomacyService.HostileThreshold - WorldWarService.AggressionSeverity,
        DiplomacyService.GetGoodwill(state, "Raiders", "Settlers"));
    AssertEqual(totalCitizens, state.Citizens.Count);
...
static void TestWorldWarScoutAttackConsequenceLoop()
{
    ...
    AssertEqual(
        DiplomacyService.HostileThreshold - WorldWarService.AggressionSeverity,
        DiplomacyService.GetGoodwill(beforeBattle, "Raiders", "Settlers"));
...
static void TestRimWorldWarResolutionAndVictoryWiring()
{
    ...
    var choiceLetter = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
        "ChoiceLetter_LivingWorldAlliance.cs"));
    AssertContains("LivingWorldFactionRelations.FormRealAlliance", choiceLetter);
    AssertContains("LivingWorldFactionRelations.FormRealEnmity", choiceLetter);
...
static void TestExpandSettlementRejectsEmptyOrInsufficientSettlers()
{
    ...
    AssertThrows<ArgumentOutOfRangeException>(
        () => state.StartSettlementExpedition(home.Id, "empty", "Empty", 0, 0, 60_000, false));
    AssertThrows<InvalidOperationException>(
        () => state.StartSettlementExpedition(home.Id, "underfilled", "Underfilled", 2, 0, 60_000, false));
    AssertEqual(1, state.Settlements.Count);
...
static void TestWorldWarCaravanTransfersRealGoods()
{
    ...
    WorldWarService.SimulateDay(state, new WorldWarRequest(120_000, TravelDays: 1, RaidCombatants: 3));
    AssertEqual(WorldTransitPhase.AtTarget, state.GetCaravan(caravan.Id)!.Phase);
    AssertEqual(0, state.GetOwnedResourceQuantity(village.Id, "Steel"));

    WorldWarService.SimulateDay(state, new WorldWarRequest(180_000, TravelDays: 1, RaidCombatants: 3));
    AssertEqual(WorldTransitPhase.Returning, state.GetCaravan(caravan.Id)!.Phase);
    AssertEqual(10, state.GetOwnedResourceQuantity(village.Id, "Steel"));
    AssertEqual(30, state.GetOwnedResourceQuantity(caravan.Id, "Silver"));
    ...
    AssertEqual(10, state.GetOwnedResourceQuantity(village.Id, "Steel"));
    AssertEqual(0, state.GetOwnedResourceQuantity(caravan.Id, "Steel"));
    AssertEqual(30, state.GetOwnedResourceQuantity(market.Id, "Silver"));
    AssertEqual(70, state.GetOwnedResourceQuantity(village.Id, "Silver"));
    AssertEqual(2, state.Events.Count(worldEvent => worldEvent.Kind == WorldEventKind.CaravanArrived));
}

static void TestWorldWarCaravanReservesAndReturnsRealCrew()
{
    var state = new WorldState(4242);
    var market = state.CreateSettlement("market", "Market", "Traders");
    for (var i = 0; i < 3; i++)
    ...
    var caravan = state.Caravans.Single();
    AssertEqual(EntityKind.Citizen, caravan.CrewCitizenId!.Value.Kind);
    AssertEqual(caravan.Id, state.GetOwner(caravan.CrewCitizenId.Value));
    AssertEqual(2, state.GetSettlementPopulation(market.Id).Total);
    ...
    AssertEqual(CaravanStatus.Arrived, state.GetCaravan(caravan.Id)!.Status);
    AssertEqual(market.Id, state.GetOwner(caravan.CrewCitizenId.Value));
    AssertEqual(CitizenStatus.Alive, state.GetCitizen(caravan.CrewCitizenId.Value)!.Status);
    AssertEqual(3, state.GetSettlementPopulation(market.Id).Total);
    AssertEqual(0, state.Validate().Count());
}

static void TestWorldWarTargetSelectorSkipsInactiveSettlements()
{
    var state = new WorldState(4242);
    ...
    var diplomacyFaction = WorldWarTargetSelector.FindDiplomacyTargetFaction(state, "Traders");
...
static void TestPersistentCaravanCrewSerialization()
{
    var state = new WorldState(4242);
    var source = state.CreateSettlement("source", "Source", "Traders");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    var crew = state.CreateCitizen("Crew", 30, Sex.Female, "merchant", source.Id);
    state.AddResource(source.Id, "Steel", 50);

    var caravan = state.CreateCaravan("Steel caravan", "Traders", source.Id, target.Id, 0, 60_000, crew.Id);
    state.TransferAsset(crew.Id, source.Id, caravan.Id, "caravan crew");
    state.TransferResource(source.Id, caravan.Id, "Steel", 20, "test cargo");

    var restored = WorldStateCodec.Deserialize(WorldStateCodec.Serialize(state));
    var restoredCaravan = restored.GetCaravan(caravan.Id)!;

    AssertEqual(crew.Id, restoredCaravan.CrewCitizenId);
    AssertEqual(caravan.Id, restored.GetOwner(crew.Id));
    AssertEqual(0, restored.GetSettlementPopulation(source.Id).Total);
    AssertEqual(0, restored.Validate().Count());
}

static void TestCaravanRecallsWhenTargetInactive()
{
    var state = new WorldState(4242);
    var source = state.CreateSettlement("source", "Source", "Traders");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    ...
    var result = CaravanMovementService.SimulateDay(state, new CaravanMovementRequest(60_000));

    AssertEqual(0, result.Arrived);
    AssertEqual(CaravanStatus.Traveling, state.GetCaravan(caravan.Id)!.Status);
    AssertEqual(WorldTransitPhase.Returning, state.GetCaravan(caravan.Id)!.Phase);
    AssertEqual(30, state.GetOwnedResourceQuantity(source.Id, "Steel"));
    AssertEqual(caravan.Id, state.GetOwner(crew.Id));

    CaravanMovementService.SimulateDay(state, new CaravanMovementRequest(120_000));

    AssertEqual(CaravanStatus.Recalled, state.GetCaravan(caravan.Id)!.Status);
    AssertEqual(50, state.GetOwnedResourceQuantity(source.Id, "Steel"));
    AssertEqual(0, state.GetOwnedResourceQuantity(target.Id, "Steel"));
    AssertEqual(source.Id, state.GetOwner(crew.Id));
    AssertEqual(CitizenStatus.Alive, state.GetCitizen(crew.Id)!.Status);
    AssertEqual(1, state.GetSettlementPopulation(source.Id).Total);
    AssertEqual(0, state.Validate().Count());
}
...
static void TestCaravanPruneRemovesTerminalCaravans()
{
    ...
    var target = state.CreateSettlement("target", "Target", "Settlers");
    state.AddResource(source.Id, "Steel", 50);
    state.AddResource(target.Id, "Silver", 100);

    var caravan = state.CreateCaravan(
        "Steel caravan", "Traders", source.Id, target.Id, 0, 60_000,
        tradeResourceKey: "Steel", requestedTradeQuantity: 20);
    state.TransferResource(source.Id, caravan.Id, "Steel", 20, "test cargo");
    CaravanMovementService.SimulateDay(state, new CaravanMovementRequest(60_000));
    CaravanMovementService.SimulateDay(state, new CaravanMovementRequest(120_000));
    CaravanMovementService.SimulateDay(state, new CaravanMovementRequest(180_000));
    AssertEqual(CaravanStatus.Arrived, state.GetCaravan(caravan.Id)!.Status);
    AssertEqual(1, state.Caravans.Count);

    // Within the retention window: the arrived caravan is kept.
    CaravanPruneService.Prune(state, new CaravanPruneRequest(180_000 + (10 * 60_000), 30));
    AssertEqual(1, state.Caravans.Count);

    // Past retention: pruned with its ownership entry — goods stay delivered, no dangling refs.
    var result = CaravanPruneService.Prune(state, new CaravanPruneRequest(180_000 + (31 * 60_000), 30));
    AssertEqual(1, result.Pruned);
...
static void TestPersistentCaravanArrivesAfterLoad()
{
    ...
    // A caravan saved mid-flight must still trade and return after load.
    var restored = WorldStateCodec.Deserialize(WorldStateCodec.Serialize(state));
    CaravanMovementService.SimulateDay(restored, new CaravanMovementRequest(60_000));
    CaravanMovementService.SimulateDay(restored, new CaravanMovementRequest(120_000));
    CaravanMovementService.SimulateDay(restored, new CaravanMovementRequest(180_000));
    var arrived = restored.GetCaravan(caravan.Id)!;
...
static void TestChangeSettlementFactionIsNoOpWhenNotActive()
{
    var state = new WorldState(12345);
    var settlement = state.CreateSettlement("worldobject:Settlement:512:Pirate", "Redwater", "Pirate");
    SettlementLifecycleService.DestroySettlement(state, settlement.Id, 100, "destroyed");
    var capturedBefore = state.Events.Count(e => e.Kind == WorldEventKind.SettlementCaptured);

    SettlementLifecycleService.ChangeSettlementFaction(state, settlement.Id, "Outlander", 200, "captured");

    AssertEqual(SettlementLifecycleStatus.Destroyed, state.GetSettlement(settlement.Id)!.Status);
    AssertEqual("Pirate", state.GetSettlement(settlement.Id)!.FactionId);
    AssertEqual(capturedBefore, state.Events.Count(e => e.Kind == WorldEventKind.SettlementCaptured));
}
...
static void TestWorldWarScoutingRecordsIntel()
{
    ...
    // Day 2: the party reaches the target; effect is applied only after surviving the arrival.
    WorldWarService.SimulateDay(state, new WorldWarRequest(120_000, TravelDays: 1, RaidCombatants: 3));
...
static void TestWorldWarDiplomatChangesGoodwill()
{
    ...
    // Day 2: the mission reaches the target but has not completed the audience yet.
...
static void TestWorldWarNonWarbandEffectsPersistThroughSaveLoad()
{
    ...
    // Let each physical action progress through target processing before saving.
    state.AssignFactionBehavior("Envoys", FactionBehavior.Excluded);
    WorldWarService.SimulateDay(state, new WorldWarRequest(6 * 60_000, TravelDays: 1, RaidCombatants: 3));
    WorldWarService.SimulateDay(state, new WorldWarRequest(7 * 60_000, TravelDays: 1, RaidCombatants: 3));

    var restored = WorldStateCodec.Deserialize(WorldStateCodec.Serialize(state));

    AssertEqual(10, restored.GetOwnedResourceQuantity(caravanTargetId, "Steel"));
    AssertEqual(null, restored.GetKnownSettlementInfo(scoutTargetId!.Value));
    AssertEqual(true, restored.HasFactionSettlementIntel("Scouts", scoutTargetId.Value));
...
static void TestRimWorldWorldConflictsSection()
{
    ...
    // Alliance decisions must not appear from an omniscient conflict scan. A conserved diplomat
    // has to arrive before the world component sends the accept/decline letter.
    AssertEqual(false, mainTab.Contains("cachedAllianceOffers", StringComparison.Ordinal));
    AssertEqual(false, mainTab.Contains("TryAddAllianceOffer", StringComparison.Ordinal));
    var worldComponent = File.ReadAllText(Path.Combine(
        FindRepoRoot(), "src", "LivingWorld.RimWorld", "LivingWorldWorldComponent.cs"));
    AssertContains("WorldEventKind.DiplomaticMissionArrived", worldComponent);
    AssertContains("processedDiplomaticArrivalMissionIds", worldComponent);
    AssertContains("mission.TargetFactionId", worldComponent);
...
static void TestRimWorldInstallScript()
{
    ...
    AssertContains("[string]$Configuration = \"Release\"", source);
    AssertContains("-c $Configuration", source);
...
static void TestRimWorldSettlementInspectPatch()
{
    ...
    // UI-5: a threat header appears only when a player-known world-war marker is marching on the
    // settlement. The movement ledger alone must not leak an undiscovered army or its ETA.
    ...
    AssertContains("IsMovementVisible(movement.ArmyId)", source);
    AssertContains("marker.IsKnownToPlayer", source);
...
static void TestRimWorldRaidIncidentPatch()
{
    ...
    AssertContains("Blocked a vanilla", source);
    AssertContains("if (ownsFactionPopulation)", source);
...
static void TestRimWorldSettlementMapFacilityDamageReconciliation()
{
    ...
    AssertContains("public static bool Prefix(Map map, out WorldObject_LivingWorldSettlementVisitSite? __state)", patch);
    AssertContains("public static void Postfix(WorldObject_LivingWorldSettlementVisitSite? __state)", patch);
...
static void TestRimWorldRealFactionRelationsBridge()
{
    ...
    // A conserved diplomat arrival opens the choice letter; accepting it bridges both real relations.
    var choiceLetter = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
    ...
    AssertContains("LivingWorldFactionRelations.FormRealEnmity", choiceLetter);
    var component = File.ReadAllText(Path.Combine(root, "src", "LivingWorld.RimWorld", "LivingWorldWorldComponent.cs"));
    AssertContains("WorldEventKind.DiplomaticMissionArrived", component);
...
static void TestRimWorldEconomyWindow()
{
    ...
    AssertContains(".Where(settlement => settlement.IsActive", window);
    AssertContains("KnownSettlementsForPlayer", window);
    AssertContains("debugExact ? state.Settlements.Where(settlement => settlement.IsActive) : KnownSettlementsForPlayer(state)", window);
    ...
    AssertContains("LivingWorldKnowledgeLabels.Population(data.PopulationBand)", window);
...
static void TestRimWorldSettlementObserverWindow()
{
    ...
    AssertContains("debugLogging", window);
    AssertContains("LW_SettlementObserver_DebugOnly", window);
    AssertContains(".Where(settlement => settlement.IsActive", window);
    AssertContains("GetSettlementPopulation", window);
    ...
    foreach (var key in new[]
    {
        ...
        "LW_SettlementObserver_DebugOnly",
...
static void TestRimWorldSettlementVisitSiteFoundation()
{
    ...
    AssertDoesNotContain("candidate.SourceSettlementWorldObjectId == sourceSettlement.ID", service);
...
static void TestRimWorldSettlementVisitCaravanRoute()
{
    ...
    AssertContains("LivingWorldSettlementVisitFloatMenuPatch.CanVisit(sourceSettlement)", action);
    AssertContains("LW_SettlementVisitUnavailableChanged", action);
...
static void TestRimWorldSettlementVisitLiveLifecycle()
{
    var root = FindRepoRoot();
    var component = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
        "LivingWorldSettlementVisitMapComponent.cs"));
    var liveSync = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
        "LivingWorldSettlementMapLiveSyncService.cs"));
    var facilityTracker = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
        "LivingWorldSettlementMapFacilityTracker.cs"));
    var siteLifecycle = File.ReadAllText(Path.Combine(
        root,
        "src",
        "LivingWorld.RimWorld",
    ...
    AssertContains("LivingWorldSettlementMapFacilityTracker.ReconcileLiveMap", liveSync);
    AssertContains("loaded settlement daily facility checkpoint", liveSync);
    ...
    AssertContains("livingWorld_facilityCheckpointSurvivors", component);
    AssertContains("GetFacilitySurvivorCheckpoint", component);
    ...
    AssertContains("livingWorld_cleanupPending", site);
    AssertContains("cleanupPending && HasMap", site);
    AssertContains("Current.Game?.DeinitAndRemoveMap", site);
...
static void TestPhysicalArmyContactDisruptsSettlerExpedition()
{
    ...
    state.DispatchArmy(army.Id, armyTarget.Id, 600_000);
    FactionConflictPolicy.DeclareWar(state, "Raiders", "Settlers", 0);
...
static void TestPhysicalArmyContactDisruptsDrifterFounding()
{
    ...
    var army = RaidPopulationAllocator.ReserveForRaid(
        ...
        new RaidPopulationAllocationRequest("Raiders", "Interception army", 4, FoodPerCitizen: 0)).Army!;
    state.DispatchArmy(army.Id, armyTarget.Id, 600_000);
    FactionConflictPolicy.DeclareWar(state, "Raiders", "Outlander", 0);

    AssertEqual(true, TransitEncounterService.TryResolvePhysicalContact(state, army.Id, journey.Id, 90_000));
    AssertEqual(DrifterFoundingJourneyStatus.Cancelled, state.GetDrifterFoundingJourney(journey.Id)!.Status);
    AssertEqual(0, state.Drifters.Count);
    AssertEqual(3, state.GetDrifterFoundingJourney(journey.Id)!.MemberOutcomes.Count(outcome =>
        outcome.Fate == DrifterFoundingMemberFate.Captured
        && outcome.CitizenId.HasValue
        && outcome.SettlementId == raiderHome.Id));
    AssertEqual(3, state.Citizens.Count(citizen =>
        citizen.SettlementId == raiderHome.Id
        && string.Equals(citizen.Profession, "captive", StringComparison.Ordinal)));
    AssertEqual(false, state.IsDrifterReserved(leader.Id));
...
static void TestRimWorldPlayerCaravanMarkerContacts()
{
    ...
    var component = File.ReadAllText(Path.Combine(root, "src", "LivingWorld.RimWorld", "LivingWorldWorldComponent.cs"));
    AssertContains(".Where(marker => !string.IsNullOrWhiteSpace(marker.MarkerKey))", component);
    AssertContains("marker.MarkerKey.StartsWith(\"migration:\"", component);
    ...
    AssertDoesNotContain(".Where(marker => marker.IsVisibleByFilter", component);
    ...
    AssertContains("LW_SettlementVisitUnavailableChanged", en);
    AssertContains("LW_SettlementVisitUnavailableChanged", ru);
}
...
static void TestRimWorldWorldTrafficFogOfWar()
{
    var root = FindRepoRoot();
    var visibilityPath = Path.Combine(root, "src", "LivingWorld.RimWorld", "LivingWorldTransitVisibility.cs");
    AssertFileExists(visibilityPath);
    ...
    AssertContains("IsPhysicallyObservedOnly", visibility);
    AssertContains("worldObject?.Faction != Faction.OfPlayer", visibility);
    AssertContains("SetStrategicVisibility", marker);
    AssertContains("IsKnownToPlayer => strategicallyVisible && IsVisibleByFilter", marker);
    AssertContains("if (!strategicallyVisible)", marker);
    AssertContains("LivingWorldTransitVisibility.IsKnown(", component);
    AssertContains("$\"migration:{group.Id.Value}\"", component);
    AssertContains("LW_MissionKind_Refugees", component);
    ...
    foreach (var key in new[]
    {
        "LW_UnknownDestination",
        "LW_MissionKind_Refugees",
        "LW_MissionReason_Refugees",
        "LW_PlayerScoutDetectedLabel",
        "LW_PlayerScoutDetectedText",
    })
    {
        AssertContains($"<{key}>", en);
        AssertContains($"<{key}>", ru);
...
static void TestWorldComponentCatchesUpMissedSimulationDays()
{
    ...
    AssertContains("the committed day will not be replayed", source);
    AssertDoesNotContain("FailedDayRetryDelayTicks", source);
...
static void TestLiveVisitAnimalCaravanDocs()
{
    ...
    AssertContains("dedicated settlement visits use stable citizen identity", taskBoard);
    AssertContains("player-taken animals stay removed", taskBoard);
...
static void TestRimWorldCaravanMeetingGate()
{
    ...
    AssertContains("Find.WorldObjects?.Settlements", patch);
    AssertContains("ResolveMeetingFaction", patch);
    AssertContains("LivingWorldVisitorBindingService.BindVisitorPawns", patch);
    AssertContains("SettlementSlug.ParseTile(ledger.Slug)", patch);
...
static void TestRimWorldApproachingVisitors()
{
    ...
    AssertContains("Disabling the visible travel animation must not disable conservation", component);
    AssertContains(": 1;", component);
...
static void TestRimWorldMechClusters()
{
    ...
    AssertContains("public int AvailableUnits", runtime);
    AssertContains("public int Steel", runtime);
    AssertContains("public int Energy", runtime);
    AssertContains("class MechRaidReservationRuntime", runtime);
    ...
    AssertContains("TryReserveMechanoidRaid", patch);
    AssertContains("MechRaidReservationRuntime.TryTake", patch);
    AssertContains("Blocked a mechanoid raid", patch);

    var component = File.ReadAllText(
        Path.Combine(root, "src", "LivingWorld.RimWorld", "LivingWorldWorldComponent.cs"));
    AssertContains("public void NotifyMechanoidRaid(IncidentParms parms)", component);
    AssertContains("public bool TryReserveMechanoidRaid", component);
    AssertContains("CompleteMechanoidRaidReservation", component);
...
static void TestDestroyedSettlementStopsAnimalSimulation()
{
    var state = new WorldState(8101);
    var settlement = state.CreateSettlement("ranch", "Ranch", "Outlander");
    state.RecordSettlementCapability(new SettlementCapability(settlement.Id, 20, 100, 20, 10, 2, 20, 0, 2, 0, 0));
    state.RecordSpecialistPool(new SpecialistPool(settlement.Id, 0, 2, 0, 1, 0, 1, 0, 0, 0));
    var herd = state.CreateAnimalCohort(
        settlement.Id,
        "Muffalo",
    ...
    state.AddResource(settlement.Id, "MedicineIndustrial", 4);
    state.AddResource(settlement.Id, "ComponentIndustrial", 3);
    var project = AnimalBreedingService.StartSelectionProject(
        state,
        new AnimalBreedingStartRequest(
            1_000,
            settlement.Id,
            herd.Id,
            AnimalBreedingTrait.Fertility,
            1_000,
            "Hay",
            10,
            "MedicineIndustrial",
    ...
    var destroyed = SettlementLifecycleService.DestroySettlement(state, settlement.Id, 1_500, "burned");
    var ruin = destroyed.Ruin!;
    var countBefore = state.GetAnimalCohort(herd.Id)!.Count;
    var foodBefore = state.GetOwnedResourceQuantity(ruin.Id, "PackagedSurvivalMeal");

    var ecology = AnimalEcologyService.SimulateDay(state, new AnimalEcologyRequest(2_500, "Hay", 1));
    var production = AnimalProductionService.SimulateDay(
        state,
        new AnimalProductionRequest(2_500, "PackagedSurvivalMeal", 2, 3, 2));
    var breeding = AnimalBreedingService.CompleteReadyProjects(state, 2_500);

    AssertEqual(ruin.Id, state.GetAnimalCohort(herd.Id)!.OwnerId);
    AssertEqual(countBefore, state.GetAnimalCohort(herd.Id)!.Count);
    AssertEqual(AnimalBreedingProjectStatus.Cancelled, state.GetAnimalBreedingProject(project.Id)!.Status);
    AssertEqual(0, ecology.Births + ecology.Deaths);
    AssertEqual(0, production.RanchFoodProduced + production.HuntingFoodProduced);
    AssertEqual(0, breeding.CompletedProjects);
    AssertEqual(foodBefore, state.GetOwnedResourceQuantity(ruin.Id, "PackagedSurvivalMeal"));
}

static void TestDestroyedSettlementRefugeesResumeMigration()
{
    var state = new WorldState(8102);
    var source = state.CreateSettlement("source", "Lost Home", "Settlers");
    var target = state.CreateSettlement("target", "Safe Home", "Settlers");
    var citizen = state.CreateCitizen("Refugee", 32, Sex.Female, "worker", source.Id);
    state.AddResource(target.Id, "Food", 200);
    SettlementLifecycleService.DestroySettlement(state, source.Id, 1_000, "destroyed");

    var launched = MigrationService.SimulateDay(
        state,
        new MigrationSimulationRequest(2_000, "Food", 1, 100, 4, 1_000));
    var group = state.MigrationGroups.Single(candidate => candidate.Reason == "destroyed-settlement-refugees");

    AssertEqual(1, launched.MigrationGroupsCreated);
    AssertEqual(CitizenStatus.Migrating, state.GetCitizen(citizen.Id)!.Status);
    AssertEqual(group.Id, state.GetOwner(citizen.Id));

    var arrived = MigrationService.SimulateDay(
        state,
        new MigrationSimulationRequest(3_000, "Food", 1, 100, 4, 1_000));
    AssertEqual(1, arrived.MigrationsCompleted);
    AssertEqual(CitizenStatus.Alive, state.GetCitizen(citizen.Id)!.Status);
    AssertEqual(target.Id, state.GetOwner(citizen.Id));
    AssertEqual(target.Id, state.GetCitizen(citizen.Id)!.SettlementId);
}

static void TestRaidPopulationAllocatorReservesEquipment()
{
    var state = new WorldState(8103);
    var settlement = state.CreateSettlement("camp", "Camp", "Pirates");
    state.CreateCitizen("Raider A", 28, Sex.Male, "soldier", settlement.Id);
    state.CreateCitizen("Raider B", 29, Sex.Female, "soldier", settlement.Id);
    state.AddResource(settlement.Id, "PackagedSurvivalMeal", 10);
    state.AddResource(settlement.Id, "Steel", 20);
    ...
    var result = RaidPopulationAllocator.ReserveForRaid(
        ...
        new RaidPopulationAllocationRequest(
            ...
            "equipped raid",
            2,
            EquipmentResourceKey: "Steel",
            EquipmentPerCombatant: 5));

    AssertEqual(RaidPopulationAllocationStatus.Success, result.Status);
    AssertEqual(10, state.GetOwnedResourceQuantity(settlement.Id, "Steel"));
    AssertEqual(10, state.GetOwnedResourceQuantity(result.Army!.Id, "Steel"));
}

static void TestMaterializationLeaseRejectsPawnRebind()
{
    var state = new WorldState(8104);
    var settlement = state.CreateSettlement("town", "Town", "Outlander");
    state.CreateCitizen("Resident", 30, Sex.Female, "worker", settlement.Id);
    var lease = MaterializationLeaseService.CreateLeases(
        state,
        new MaterializationLeaseRequest(
            settlement.Id,
            ...
            "visit:town",
            1,
            60_000)).Leases.Single();

    var first = MaterializationLeaseService.BindPawn(state, lease.Id, 1001);
    var idempotent = MaterializationLeaseService.BindPawn(state, lease.Id, 1001);
    var rejected = MaterializationLeaseService.BindPawn(state, lease.Id, 1002);

    AssertEqual(MaterializationLeaseBindStatus.Success, first.Status);
    AssertEqual(MaterializationLeaseBindStatus.Success, idempotent.Status);
    AssertEqual(MaterializationLeaseBindStatus.PawnAlreadyLeased, rejected.Status);
    AssertEqual(1001, state.GetMaterializationLease(lease.Id)!.PawnThingId);
}

static void TestFactionLifecycleCollapsesAfterLastCapture()
{
    var state = new WorldState(8105);
    var settlement = state.CreateSettlement("last", "Last Home", "OldFaction");
    state.CreateCitizen("Resident", 30, Sex.Male, "worker", settlement.Id);
    state.AssignFactionBehavior("OldFaction", FactionBehavior.Cautious);
    state.CaptureSettlement(settlement.Id, "Captors");

    var result = FactionLifecycleService.SimulateCollapses(
        state,
        new FactionLifecycleRequest(60_000, new[] { "OldFaction" }));

    AssertEqual(1, result.CollapsedFactions);
    AssertEqual(WorldFactionStatus.Collapsed, state.GetFactionRecord("OldFaction")!.Status);
}

static void TestTerminalArmyReleasesDeadOwnership()
{
    var state = new WorldState(8106);
    var source = state.CreateSettlement("home", "Home", "Raiders");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    var citizen = state.CreateCitizen("Fallen", 31, Sex.Male, "soldier", source.Id);
    var army = state.CreateArmy("Army", "Raiders", source.Id);
    state.TransferAsset(citizen.Id, source.Id, army.Id, "deployed");
    state.MarkCitizenDead(citizen.Id, "killed in battle");
    state.DispatchArmy(army.Id, target.Id, 60_000);
    ...
    AssertEqual(citizen.Id, state.GetOwner(citizen.Id));
    AssertEqual(CitizenStatus.Dead, state.GetCitizen(citizen.Id)!.Status);
    AssertEqual(0, state.GetCitizensOwnedBy(army.Id).Count);
}

static void TestRaidRecallAvoidsCapturedSource()
{
    var state = new WorldState(8107);
    var source = state.CreateSettlement("source", "Source", "Raiders");
    var fallback = state.CreateSettlement("fallback", "Fallback", "Raiders");
    state.CreateCitizen("Raider A", 28, Sex.Male, "soldier", source.Id);
    state.CreateCitizen("Raider B", 29, Sex.Female, "soldier", source.Id);
    state.AddResource(source.Id, "PackagedSurvivalMeal", 10);
    state.AddResource(source.Id, "Steel", 10);
    var allocation = RaidPopulationAllocator.ReserveForRaid(
        state,
        new RaidPopulationAllocationRequest(
    ...
    AssertEqual(2, released);
    AssertEqual(2, state.GetSettlementPopulation(fallback.Id).Total);
    AssertEqual(2, state.GetOwnedResourceQuantity(fallback.Id, "PackagedSurvivalMeal"));
    AssertEqual(10, state.GetOwnedResourceQuantity(fallback.Id, "Steel"));
    AssertEqual(0, state.ResourcesForOwner(allocation.Army.Id).Count);
}

static void TestCaravanRecallsFromHostileCaptor()
{
    var state = new WorldState(8108);
    var source = state.CreateSettlement("source", "Source", "Traders");
    var target = state.CreateSettlement("target", "Target", "Settlers");
    state.AddResource(source.Id, "Steel", 20);
    var caravan = state.CreateCaravan(
        "Steel caravan",
        "Traders",
        source.Id,
        ...
        0,
        60_000,
        tradeResourceKey: "Steel",
        requestedTradeQuantity: 10);
    state.TransferResource(source.Id, caravan.Id, "Steel", 10, "cargo");
    state.CaptureSettlement(target.Id, "HostileCaptors");
    DiplomacyService.AdjustGoodwill(state, "Traders", "HostileCaptors", -100);
