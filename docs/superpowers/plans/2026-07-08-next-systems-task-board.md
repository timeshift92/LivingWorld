...
| Task 4 | Facilities & settlement projects | Codex core + Claude bands | ◐ | Core foundation landed (Codex): `SettlementFacility`, `SettlementProject`, build/repair projects consume real materials, facilities affect production, save/load covered. **RW bands/inspection landed (Claude):** the settlement inspect panel now surfaces facilities + an active build/repair project, fog-of-war gated (exact levels/condition/ETA only when directly-known, else a coarse development band + generic "developing" note); EN/RU + `TestRimWorldSettlementFacilitiesInspection`. **Remaining (Codex/Core):** a *generation driver* — nothing yet builds facilities or starts projects during simulation, so `GetSettlementFacilities` is empty in-game and the new UI has nothing to show until settlements deterministically invest over time. |
| Task 5 | Ruins & relocation | Codex core + Claude markers | ◐ | Core foundation landed: settlement `Active/Abandoned/Destroyed` status, `WorldRuin`, destruction -> refugees + salvage owner, relocation via migration group with resource transfer, reclaim/prune lifecycle, save/load covered. RW markers/inspect still pending. |
