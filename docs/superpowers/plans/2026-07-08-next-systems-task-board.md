...
| Task 7 | Animal cohorts & ecology | Codex core | ◐ Core seeding/wiring done | Core foundation landed: `WorldAnimalCohort` stores animals as owned lightweight ledger cohorts, `AnimalEcologyService` simulates deterministic growth/decline from carrying capacity and feed pressure, pressure migration moves animals between owners without duplication, save/load covered. **Core driver landed:** `AnimalEcologyDriver.SimulateDay` deterministically seeds missing wild/domesticated cohorts from settlement biome, temperature, rainfall, growing days, and `AnimalCapacity`, then runs feed pressure against the real food ledger. **RW wired:** the daily RimWorld tick now calls the driver, so cohorts appear and consume food in-game instead of staying as dormant Core data. **RW visibility landed (Claude):** the settlement inspect panel now surfaces animal cohorts (`state.AnimalCohorts` by owner) with the same fog-of-war gating as production/facilities — exact per-kind herd sizes when directly known, else a coarse abundance band (sparse/moderate/abundant/teeming); EN/RU + `TestRimWorldSettlementAnimalsInspection`. Remaining: explicit hunting/ranching output rules (Core), and materialization when animals appear on maps (bigger, like Task 3). |
| Task 8 | Technology, selection & incubation | Codex core + Claude gates | ◐ Core foundation done | Core foundation landed: `AnimalBreedingProject` + `AnimalBreedingService` run ledger-first selection/incubation projects over animal cohorts, require settlement animal/lab capability + specialists, consume real feed/medicine/components, complete by tick, improve cohort traits or create incubated ledger cohorts, save/load covered. Remaining: crop strains, technology diffusion tiers, RW visibility/gates, Biotech-specific adapters later. |
